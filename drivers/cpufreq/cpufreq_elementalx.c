...
#define DEF_ACTIVE_FLOOR_FREQ			(1026000)
#define DEF_GBOOST_MIN_FREQ			(1242000)
#define DEF_MAX_SCREEN_OFF_FREQ			(1026000)
...
static struct ex_dbs_tuners ex_tuners = {
	.up_threshold = DEF_FREQUENCY_UP_THRESHOLD,
	.down_differential = DEF_FREQUENCY_DOWN_DIFFERENTIAL,
	.active_floor_freq = DEF_ACTIVE_FLOOR_FREQ,
	.gboost_min_freq = DEF_GBOOST_MIN_FREQ,
...
static ssize_t show_sampling_rate_min(struct dbs_data *dbs_data, const char *buf)
{
...
show_one(ex, sampling_rate, sampling_rate);
show_one(ex, up_threshold, up_threshold);
show_one(ex, down_differential, down_differential);
show_one(ex, gboost, gboost);
show_one(ex, gboost_min_freq, gboost_min_freq);
show_one(ex, active_floor_freq, active_floor_freq);
show_one(ex, max_screen_off_freq, max_screen_off_freq);
show_one(ex, sampling_down_factor, sampling_down_factor);

define_one_global_rw(sampling_rate);
define_one_global_rw(up_threshold);
define_one_global_rw(down_differential);
define_one_global_rw(gboost);
define_one_global_rw(gboost_min_freq);
define_one_global_rw(active_floor_freq);
define_one_global_rw(max_screen_off_freq);
...
static struct attribute *dbs_attributes[] = {
	...
	&gboost.attr,
	&gboost_min_freq.attr,
	&active_floor_freq.attr,
	&max_screen_off_freq.attr,
	&sampling_down_factor.attr,
	NULL
};

static struct attribute_group ex_attr_group = {
...
static struct dbs_data ex_dbs_data = {
	.governor = GOV_ELEMENTALX,
	.attr_group = &ex_attr_group,
	.tuners = &ex_tuners,
...
static int ex_cpufreq_governor_dbs(struct cpufreq_policy *policy,
				   unsigned int event)
{
	return cpufreq_governor_dbs(policy, &ex_dbs_data, event);
}
